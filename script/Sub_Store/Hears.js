//https://raw.githubusercontent.com/xream/scripts/main/surge/modules/sub-store-scripts/sub-info/headers.js 大佬链接获得
  ...
  return (
    ['GET', 'POST', 'PUT', 'DELETE', 'HEAD', 'OPTIONS', 'PATCH'].forEach(
      c =>
        (u[c.toLowerCase()] = u =>
          (function (u, c) {
            ;(c = 'string' == typeof c ? { url: c } : c).url = e ? e + c.url : c.url
            const h = (c = { ...t, ...c }).timeout,
              l = { onRequest: () => {}, onResponse: e => e, onTimeout: () => {}, ...c.events }
            let a, d
            if ((l.onRequest(u, c), s)) a = $task.fetch({ method: u, ...c })
            else if (o || i || r)
              a = new Promise((e, t) => {
                ;(r ? require('request') : $httpClient)[u.toLowerCase()](c, (s, o, i) => {
            ...
            const f = h
              ? new Promise((e, t) => {
                  d = setTimeout(() => (l.onTimeout(), t(`${u} URL: ${c.url} exceeds the timeout ${h} ms`)), h)
                })
              : null
            return (f ? Promise.race([f, a]).then(e => (clearTimeout(d), e)) : a).then(e => l.onResponse(e))
          })(c, u))
    ),
    u
  )
  ...
  return new (class {
    ...
    initCache() {
      if (
        ...
        (o || i) && (this.cache = JSON.parse($persistentStore.read(this.name) || '{}')),
        n)
      ) {
        let e = 'root.json'
        this.node.fs.existsSync(e) ||
          this.node.fs.writeFileSync(e, JSON.stringify({}), { flag: 'wx' }, e => console.log(e)),
          (this.root = {}),
          (e = `${this.name}.json`),
          this.node.fs.existsSync(e)
    ...
    persistCache() {
      ...
      s && $prefs.setValueForKey(e, this.name),
        ...
        n &&
          (this.node.fs.writeFileSync(`${this.name}.json`, e, { flag: 'w' }, e => console.log(e)),
          this.node.fs.writeFileSync('root.json', JSON.stringify(this.root), { flag: 'w' }, e => console.log(e)))
    }
    write(e, t) {
      this.log(`SET ${t}`),
        -1 !== t.indexOf('#')
    ...
    read(e) {
      return (
        ...
        -1 === e.indexOf('#')
          ? this.cache[e]
          : ((e = e.substr(1)),
            i & o ? $persistentStore.read(e) : s ? $prefs.valueForKey(e) : n ? this.root[e] : void 0)
      )
    }
    delete(e) {
      this.log(`DELETE ${e}`),
    ...
    notify(e, t = '', c = '', h = {}) {
      const l = h['open-url'],
        a = h['media-url']
      if ((s && $notify(e, t, c, h), i && $notification.post(e, t, c + `${a ? '\n多媒体:' + a : ''}`, { url: l }), o)) {
        let s = {}
        l && (s.openUrl = l),
          a && (s.mediaUrl = a),
          '{}' == JSON.stringify(s) ? $notification.post(e, t, c) : $notification.post(e, t, c, s)
      }
      if (n || u) {
        const s = c + (l ? `\n点击跳转: ${l}` : '') + (a ? `\n多媒体: ${a}` : '')
        if (r) {
    ...
    wait(e) {
      ...
      s || o || i
        ...
        : n &&
          ...
          (($context.headers = e.headers), ($context.statusCode = e.statusCode), ($context.body = e.body))
    }
  })(e, t)
}
/*****************************************************************************/
